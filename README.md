...
* [`src/todomvc_reframe_shadow_cljs/`](src/todomvc_reframe_shadow_cljs/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/todomvc_reframe_shadow_cljs/core.cljs): contains the SPA entry point, `init`
* [`test/todomvc_reframe_shadow_cljs/`](test/todomvc_reframe_shadow_cljs/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `todomvc-reframe-shadow-cljs` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/todomvc_reframe_shadow_cljs/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns todomvc-reframe-shadow-cljs.example
  (:require [todomvc-reframe-shadow-cljs.config :as config])
